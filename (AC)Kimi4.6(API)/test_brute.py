...
def brute(n):
    ...
    for idx in range(3, n + 1):
        ...
        for i in range(1, idx):
            diffs.add(a[idx] - a[i])
    total = sum(a[1:n+1]) % MOD
    return total

# test random n up to 200
max_n = 200
max_k = max_n // 2
U, prefU, P = precompute(max_k)
for n in range(1, max_n + 1):
    expected = brute(n)
    got = solve_one(n, U, prefU, P)
    if expected != got:
        print('Mismatch at', n, expected, got)
        sys.exit(1)
print('All ok up to', max_n)

# test a few larger random
for _ in range(20):
