# SoTA-Algorithm-Benchmarks
[English](#english) | [中文](#chinese)

<a name="english"></a>
## SoTA-Algorithm-Benchmarks

| AI                             |   OJ    | Result |Thinking Time(min)|
| :----------------------------- | :-----: | :--: | ---- |
| Deepseek(Web) Expert               | CoT Cycle |  ❌   |N/A|
| Doubao(Web) Expert                |   RE    |  ❌   |~10|
| Qwen-3.6-Plus(Web)             |   WA    |  ❌   |~20|
| Qwen-3.6-Max-Preview(Web)            |   TLE    |  ❌   |~20|
| Qwen-3.6-Plus(API)             |   WA    |  ❌   |~120|
| Qwen-3.6-Max-Preview(API)             |   Exceed Context    |  ❌   |N/A|
| Kimi2.6(Web)        |   No output    |  ❌   |N/A|
| Minimax2.7             |   No output    |  ❌   |N/A|
| GLM-5.1(API)        |   AC    |  ✅   |~120|
| Kimi2.6(API)       |   AC  |  ℹ️  |~120|
| GPT-5.4-Thinking(Extend)       |   TLE   |  ❌   |101|
| GPT-5.4-Thinking(Heavy)        |   AC    |  ✅   |~60|
| GPT-5.4-Pro Standard |   AC    |  ✅   |~60|
| (New?)GPT-5.4-Pro Standard |   AC    |  ✅   |~20|
| GPT-5.4-Pro Extended |   AC    |  ✅   |~60|
| GPT-5.4-high                  |   AC    |  ✅   |~20|
| GPT-5.4-xhigh                  |   AC    |  ✅   |~30|
| Gemini-3.1-Pro                 |   TLE   |  ❌   |~60|
| Gemini-3.1-Pro-Deepthink       |   AC    |  ✅   |~60|
| Claude-Opus-4.7-Max(API)       |   AC    |  ✅   |~100|
| Grok-4.2-Heavy      |   RE    |  ❌   |12|

---

<a name="chinese"></a>
## SoTA算法能力排行榜

| AI                             |   OJ    | 得分 |思考时间(min)|
| :----------------------------- | :-----: | :--: | ---- |
| Deepseek专家模式               | CoT循环 |  ❌   |\\|
| 豆包Web专家模式                |   RE    |  ❌   |~10|
| Qwen-3.6-Plus 思考             |   WA    |  ❌   |~20|
| Qwen-3.6-Max-Preview 思考             |   TLE    |  ❌   |~20|
| Qwen-3.6-Plus(API)             |   WA    |  ❌   |~120|
| Qwen-3.6-Max-Preview(API)             |   爆上下文    |  ❌   |\\|
| Kimi2.6(Web)        |   无输出    |  ❌   |\\|
| Minimax2.7             |   无输出    |  ❌   |\\|
| GLM-5.1(API)        |   AC    |  ✅   |~120|
| Kimi2.6(API)       |   AC  |  ℹ️  |~120|
| GPT-5.4-Thinking(Extend)       |   TLE   |  ❌   |101|
| GPT-5.4-Thinking(Heavy)        |   AC    |  ✅   |~60|
| GPT-5.4-Pro Standard |   AC    |  ✅   |~60|
| (New?)GPT-5.4-Pro Standard |   AC    |  ✅   |~20|
| GPT-5.4-Pro Extended |   AC    |  ✅   |~60|
| GPT-5.4-high                  |   AC    |  ✅   |~20|
| GPT-5.4-xhigh                  |   AC    |  ✅   |~30|
| Gemini-3.1-Pro                 |   TLE   |  ❌   |~60|
| Gemini-3.1-Pro-Deepthink       |   AC    |  ✅   |~60|
| Gemini-3.1-Pro(API)                 |   WA   |  ❌   |~120|
| Claude-Opus-4.7-Max(API)       |   AC    |  ✅   |~100|
| Grok-4.2-Heavy      |   RE    |  ❌   |12|





